...
class RegisterUser(MethodView):
    ...
    def post(self):
        ...
        response = val_data.validate_register_data(
            username, email, password, isAdmin)
        if response:
            return jsonify(response), 400
        else:
            new_user = User(username, email, password, isAdmin)
            db_connection.create_new_user(
                user_id, username, email, password, isAdmin)
...
class LoginUser(MethodView):
    ...
    def post(self):
        ...
        if response:
            ...
            ret_u["email"] = returned_user[2]
            ret_u["password"] = returned_user[3]
            if password != ret_u["password"]:
...
auth.add_url_rule("/api/v1/user/register",
                  view_func=registration_view, methods=['POST'])
